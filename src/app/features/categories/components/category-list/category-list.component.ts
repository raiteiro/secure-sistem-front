import { Component, computed, inject, OnInit, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { TableModule } from 'primeng/table';
import { DialogModule } from 'primeng/dialog';
import { ConfirmDialogModule } from 'primeng/confirmdialog';
import { ConfirmationService } from 'primeng/api';
import { HasPermissionDirective } from '../../../../shared/directives/has-permission.directive';
import { CategoriesService } from '../../services/categories.service';
import { ProductsService } from '../../../products/services/products.service';
import { CompaniesService } from '../../../companies/services/companies.service';
import { AuthService } from '../../../../core/services/auth.service';
import { ICategory, ICategoryRequest } from '../../../../core/models/category.models';
import { IProduct } from '../../../../core/models/product.models';
import { ICompany } from '../../../../core/models/company.models';
import { NotificationService } from '../../../../core/services/notification.service';

interface ICategoryForm {
  name: string;
  description: string;
  /** Solo se usa al crear, y solo si el que llama es admin de sistema */
  companyId: number | null;
}

const EMPTY_FORM: ICategoryForm = { name: '', description: '', companyId: null };

interface ICategoryGroup {
  companyId: number;
  companyName: string;
  categories: ICategory[];
}

@Component({
  selector: 'app-category-list',
  standalone: true,
  imports: [CommonModule, FormsModule, TableModule, DialogModule, ConfirmDialogModule, HasPermissionDirective],
  providers: [ConfirmationService],
  templateUrl: './category-list.component.html',
  styleUrl: './category-list.component.scss'
})
export class CategoryListComponent implements OnInit {
  private readonly categoriesService = inject(CategoriesService);
  private readonly productsService = inject(ProductsService);
  private readonly companiesService = inject(CompaniesService);
  private readonly confirmService = inject(ConfirmationService);
  private readonly notify = inject(NotificationService);
  readonly authService = inject(AuthService);

  categories = signal<ICategory[]>([]);
  loading = signal(true);
  companies = signal<ICompany[]>([]);
  products = signal<IProduct[]>([]);

  showModal = false;
  isEdit = signal(false);
  saving = signal(false);
  editId = 0;
  form: ICategoryForm = { ...EMPTY_FORM };

  ngOnInit(): void {
    this.loadCategories();
    this.productsService.getAll().subscribe({
      next: (data) => this.products.set(data),
      error: () => this.notify.error('No se pudieron cargar los productos')
    });

    if (this.authService.isSystemAdmin()) {
      this.companiesService.getAll().subscribe({
        next: (data) => this.companies.set(data),
        error: () => this.notify.error('No se pudieron cargar las empresas')
      });
    }
  }

  loadCategories(): void {
    this.loading.set(true);
    this.categoriesService.getAll().subscribe({
      next: (data) => { this.categories.set(data); this.loading.set(false); },
      error: () => { this.notify.error('No se pudieron cargar las categorías'); this.loading.set(false); }
    });
  }

  companyName(companyId?: number): string {
    if (!companyId) return '—';
    return this.companies().find((c) => c.id === companyId)?.name ?? `#${companyId}`;
  }

  /** No se puede desactivar una categoría con productos activos asignados — se deshabilita antes de intentarlo */
  hasActiveProducts(category: ICategory): boolean {
    return this.products().some((p) => p.categoryId === category.id && p.isActive);
  }

  /** Separa la tabla por empresa cuando quien mira es admin de sistema; para el resto es un único grupo */
  readonly categoryGroups = computed<ICategoryGroup[]>(() => {
    const categories = this.categories();
    if (!this.authService.isSystemAdmin()) {
      return [{ companyId: 0, companyName: '', categories }];
    }

    const byCompany = new Map<number, ICategory[]>();
    for (const category of categories) {
      const companyId = category.companyId ?? 0;
      const list = byCompany.get(companyId) ?? [];
      list.push(category);
      byCompany.set(companyId, list);
    }

    return [...byCompany.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([companyId, list]) => ({
        companyId,
        companyName: this.companyName(companyId),
        categories: list
      }));
  });

  // ===== Crear / editar =====
  /** `companyId`: al crear desde el botón "+" de un grupo, preselecciona esa empresa */
  onCreate(companyId?: number): void {
    this.form = { ...EMPTY_FORM, companyId: companyId ?? null };
    this.isEdit.set(false);
    this.editId = 0;
    this.showModal = true;
  }

  onEdit(category: ICategory): void {
    this.isEdit.set(true);
    this.editId = category.id;
    this.form = { name: category.name, description: category.description ?? '', companyId: null };
    this.showModal = true;
  }

  onSave(): void {
    if (!this.form.name) {
      this.notify.warn('El nombre es obligatorio');
      return;
    }

    const request: ICategoryRequest = {
      name: this.form.name,
      description: this.form.description || null,
      ...(this.form.companyId ? { companyId: this.form.companyId } : {})
    };

    this.saving.set(true);
    const obs = this.isEdit()
      ? this.categoriesService.update(this.editId, request)
      : this.categoriesService.create(request);

    obs.subscribe({
      next: () => {
        this.saving.set(false);
        this.showModal = false;
        this.notify.success(this.isEdit() ? 'Categoría actualizada' : 'Categoría creada');
        this.loadCategories();
      },
      error: (e) => { this.saving.set(false); this.notify.httpError(e); }
    });
  }

  onDeactivate(category: ICategory): void {
    if (this.hasActiveProducts(category)) {
      this.notify.warn('No puedes desactivar una categoría con productos activos asignados');
      return;
    }

    this.confirmService.confirm({
      message: `¿Desactivar la categoría "${category.name}"?`,
      header: 'Confirmar',
      icon: 'pi pi-exclamation-triangle',
      acceptLabel: 'Sí',
      rejectLabel: 'Cancelar',
      accept: () => {
        this.categoriesService.deactivate(category.id).subscribe({
          next: () => { this.notify.success('Categoría desactivada'); this.loadCategories(); },
          error: (e) => this.notify.httpError(e)
        });
      }
    });
  }
}
